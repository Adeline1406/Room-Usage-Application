...
function statisticsGenerator(buildingProp) {
	...
	for (let index = 0; index < numObservations; index++) {
		...
		if ((buildingProp._roomList[index]._seatsUsed === 0) && (buildingProp._roomList[index]._lightsOn === true || buildingProp._roomList[index]
				._heatingCoolingOn === true)) {
			numWasteful += 1
		}
	}
...
if (roomUsageListStorage !== null) {
	...
	for (let prop in building) {
		...
		let newBuilding = "<div class=\"mdl-cell mdl-cell--4-col" + redTable(buildingStats[2], 300) +
			"\"><table class=\"observation-table mdl-data-table mdl-js-data-table mdl-shadow--2dp" + redTable(buildingStats[2], 300) + "\"><thead" + redTable(
				buildingStats[2], 300, true) + "><tr" + redTable(buildingStats[2], 300, true) + "><th class=\"mdl-data-table__cell--non-numeric" + redTable(
				buildingStats[2], 700) + "\"><h4>" + buildingStats[0] + "</h4></th></tr></thead><tbody" + redTable(buildingStats[2], 500, true) + "><tr" +
			redTable(buildingStats[2], 500, true) + "><td class=\"mdl-data-table__cell--non-numeric" + redTable(buildingStats[2], 300) + "\">" +
